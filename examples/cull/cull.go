...
//
// The cull kernel's WGSL is generated by the Elio compiler from
// ir.CullKernel() (emit/wgsl) — so this example exercises the whole pipeline:
// Elio IR → WGSL → GoSX compute pass → bus → indirect draw.
package cull
...
func New(dev gpu.Device, id string) (*Pass, error) {
	src, err := wgsl.Emit(ir.CullKernel())
	if err != nil {
		return nil, fmt.Errorf("elio/cull.New: emit wgsl: %w", err)
	}
	shader, err := dev.CreateShaderModule(gpu.ShaderDesc{SourceWGSL: src, Label: "elio.cull:" + id})
	if err != nil {
